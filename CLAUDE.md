...
### 알려진 미해결 모바일 인쇄 문제
- **GitHub Pages + 모바일**: 새 창은 열리지만 인쇄 다이얼로그가 자동으로 뜨지 않는 경우 있음
  - `win.onload` 후 `win.print()` 호출하지만, 일부 모바일 브라우저에서 보안 정책상 차단될 수 있음
- **사파리 모바일**: 스타일(셀 높이 등)이 크롬과 다르게 적용되어 60개 단어가 한 페이지에 들어가지 않음
  - 사파리는 `@media print` 및 CSS 높이 처리 방식이 크롬과 상이함, 현재 해결책 없음
